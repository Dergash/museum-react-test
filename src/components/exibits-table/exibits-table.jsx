...
export default class ExibitsTable extends React.Component {
    ...
    render() {
        const byFilters = exibit => !this.props.filters.length || this.props.filters.indexOf(exibit.origin) > -1;
        const bySearchPattern = exibit => !this.props.searchPattern.length
                || exibit.name.toUpperCase().indexOf(this.props.searchPattern.toUpperCase()) > -1;
        const byPagination = (exibit, index) => {
            const firstOnPage = (this.state.page - 1) * this.props.itemsOnPage;
            const lastOnPage = firstOnPage + this.props.itemsOnPage - 1;
        ...
        return (
            <div className={cn.mix(this.props.className)}>
                <Table hover>
                    {this.renderHeader()}
                    {this.renderBody(exibitsToShow)}
                    {this.renderFooter(exibitsToWork, exibitsToShow)}
                </Table>
            </div>
        );
    }
    ...
    @autobind
    renderFooter(exibitsToWork, exibitsToShow) {
        return (
            <tfoot>
                <tr>
                    <td colSpan="4">
